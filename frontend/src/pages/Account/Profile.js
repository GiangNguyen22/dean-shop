...
const Profile = () => {
  ...
  return (
    <div>
              ...
              {userInfo?.addressList?.map((address, index) => {
                return (
                  ...
                  >
                    ...
                    <p className="pb-2">{address?.phone}</p>
                    <p className="pb-2">
                      {address?.street},{address?.city},{address?.state}
                    </p>
                    <p>{address?.zipCode}</p>
                    <div className="flex gap-2">
                      <button onClick={() =>setEditAddress(address)} className="underline text-blue-900">Edit</button>
      ...
      {editAddress && <EditAddress onCancel={()=> setEditAddress(null)}
