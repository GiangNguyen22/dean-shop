...
const EditAddress = ({onCancel, initialData}) => {
...
const onSubmit = (evt)=>{
    ...
    setError('');
    updateAddressAPI(values, initialData?.id).then(res =>{
...
const handleOnChange = (e)=>{
  ...
  return (
    <div>
        <p className='text-xl pt-4'>Edit Address</p>
        <form onSubmit={onSubmit} className='pt-2 mb-2 md:w-[420px] w-full'>
            ...
            <input type='text' name='phone' value={values?.phone} onChange={handleOnChange} placeholder='Contact number'
             className='w-full border p-2 my-2 border-gray-400' required/>
             <label>Address</label>
