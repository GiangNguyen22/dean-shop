...
import { deleteItemFromCartAction, updateItemToCartAction } from '../../store/actions/cartActions';
            ...
            <tbody>
                {
                    cartItems?.map((item,index)=>{
                        return (
                            <tr key={`${item?.productId} - ${item?.variant?.id}`} className='p-4 bg-white border-b'>
                                <td>
